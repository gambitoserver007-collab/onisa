...
-- ============================================================
-- Pago dividido: una venta puede pagarse con más de un método (ej. parte
-- efectivo, parte tarjeta). sale_payments guarda el desglose real; si el
-- cajero no divide el pago, create_sale sigue escribiendo una sola fila
-- (mismo comportamiento de siempre, solo que ahora vive en esta tabla en
-- vez de inferirse de sales.payment_method/total).
--
-- Se define aquí (antes de sales_by_payment_method) y no junto al resto
-- de las adiciones de esta etapa al final del archivo, porque
-- sales_by_payment_method es "language sql": Postgres valida que las
-- tablas referenciadas existan al CREAR la función, no hasta que se llama
-- -- a diferencia de las funciones plpgsql (create_sale,
-- compute_cash_session_expected, submit_till_count), que resuelven los
-- nombres hasta la ejecución.
-- ============================================================
...
drop policy if exists "sale_payments select scoped" on public.sale_payments;
create policy "sale_payments select scoped" on public.sale_payments for select to authenticated
  using (public.can_select_company(company_id, is_demo_data));
-- Sin política de insert/update/delete para "authenticated": el desglose de
-- pago es dinero real, solo se escribe vía create_sale (SECURITY DEFINER),
-- mismo principio que loyalty_ledger/till_counts.
...
language sql stable security definer set search_path = public as $$
  ...
  select coalesce(sp.method, 'Otro') as method,
         sum(sp.amount)::numeric as total,
...
-- ============================================================
-- Promociones automáticas: condición de cantidad mínima, aplicada a un
-- producto puntual o a toda una categoría (sumando cantidades de todos los
-- productos/variantes de esa categoría en el carrito). Antes, "promotions"
-- era solo un catálogo informativo -- create_sale nunca lo leía y el
-- cajero no tenía forma de aplicar un descuento a mano. Por ahora solo
-- promotion_type='discount' se aplica solo; 2x1/combo se quedan
-- informativos (mecánica distinta, no cabe en "% sobre el precio").
-- ============================================================

alter table public.promotions add column if not exists scope_type text not null default 'none';
alter table public.promotions add column if not exists product_id uuid references public.products(id) on delete cascade;
alter table public.promotions add column if not exists category_id uuid references public.categories(id) on delete cascade;
alter table public.promotions add column if not exists min_qty numeric(12,3);

do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'promotions_scope_type_chk') then
    alter table public.promotions add constraint promotions_scope_type_chk
      check (scope_type in ('none','product','category'));
  end if;
end $$;
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'promotions_scope_consistency_chk') then
    alter table public.promotions add constraint promotions_scope_consistency_chk
      check (
        (scope_type = 'none' and product_id is null and category_id is null)
...
-- create_sale: además de precios/lealtad, ahora evalúa promociones
-- automáticas por cantidad. Se calcula ANTES del cálculo de IVA de cada
-- línea (reduce el precio unitario efectivo), para que el IVA se calcule
-- sobre el monto ya descontado -- igual de correcto que si el producto
-- tuviera ese precio de catálogo. Si compiten varias promociones sobre el
-- mismo producto (una por producto y otra por su categoría, por ejemplo),
-- gana la de mayor porcentaje; nunca se acumulan dos promociones en la
-- misma línea.
create or replace function public.create_sale(
  p_customer_id uuid,
  p_document_type text,
...
-- ============================================================
-- Pago dividido: create_sale ahora acepta un desglose de sale_payments
-- (ver definición de la tabla más arriba, antes de sales_by_payment_method,
-- porque esa función es "language sql" y valida las tablas referenciadas
-- al crearse, no hasta ejecutarse).
-- ============================================================

drop function if exists public.create_sale(uuid, text, text, jsonb, uuid, uuid, uuid, integer);
create or replace function public.create_sale(
  p_customer_id uuid,
  p_document_type text,
  p_payment_method text,
...
begin
  ...
  -- Pago dividido: si el cajero manda un desglose, se valida que sume
  -- exacto al total ya final (post-promociones y post-lealtad) -- nunca se
  -- confía en los montos individuales, solo se acepta la combinación si
  -- cuadra con lo que create_sale ya calculó. Sin desglose, se guarda una
  -- sola fila (mismo comportamiento de siempre). Si el total quedó en 0
  -- (cubierto por completo con puntos), no hay nada que pagar.
  if v_total > 0 then
    if p_payments is not null and jsonb_array_length(p_payments) > 0 then
      ...
      for v_payment_item in select * from jsonb_array_elements(p_payments) loop
        ...
        insert into public.sale_payments (company_id, sale_id, method, amount)
        values (v_company_id, v_sale_id, v_payment_method, round(v_payment_amount, 2));
      end loop;
      ...
      insert into public.sale_payments (company_id, sale_id, method, amount)
      values (v_company_id, v_sale_id, coalesce(p_payment_method, 'Efectivo'), v_total);
    end if;
  end if;
